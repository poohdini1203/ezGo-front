...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      ...
      passengerTripID:options.passengerTripID
    })
    this.loadData()
  },
  ...
  getDistance(){
    ...
    joinDriver(){
      let that=this
      if(!that.data.sentInvitation){
        let passengerTripID=that.data.passengerTripID
      new Promise((resolve,reject)=>{
        //乘客加入
         if(passengerTripID==null||passengerTripID.length==0){
           //发送乘客订单信息,跳转
           let str=JSON.stringify(that.data.tripInfo)
           wx.navigateTo({
             url: '../joinDriverTrip/joinDriverTrip?str='+str,
             success: (result)=>{
         ...
         //乘客邀请司机
          ...
          wx.request({
            url:app.globalData.IP+'/mapping/passengerInviteDriver',
            data:{
              'passengerTripID':that.data.passengerTripID,
